...
class FactoryExampleTest < Calificador::Test
  ...
  must "prefer trait over property", trait: :trait do
  ...
  must "prefer value over property", property: :something do
  ...
  must "prefer value over trait and property", trait: :trait, property: :something do
    assert { subject.property } == :something
  ...
  with "value", property: :something do
  ...
  with "trait and value", trait: :trait, property: :something do
  ...
  with "value for factory", FactoryExample => { property: :other } do
    must "override factory" do
      assert { create(FactoryExample).property } == :other
    end
  end

  with "value for factory with trait", property: :override, FactoryExample.with(:trait) => { property: :other } do
    must "override factory" do
